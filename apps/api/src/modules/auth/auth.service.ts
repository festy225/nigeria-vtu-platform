...
export class AuthService {
  ...
  async register(input: { email?: string; phone?: string; password: string }, request: Request) {
    ...
    const result = await this.db.withTransaction(async (client) => {
      const inserted = await client.query<{ id: string; email: string | null; phone: string | null }>(
        `INSERT INTO users (email, phone, password_hash, status) VALUES ($1,$2,$3,'ACTIVE') RETURNING id,email,phone`, [email, phone, passwordHash]
      );
  ...
  async login(identifier: string, password: string, request: Request) {
    const result = await this.db.query<{ id: string; email: string | null; phone: string | null; password_hash: string; status: string; roles: UserRole[] }>(
      `SELECT u.id,u.email,u.phone,u.password_hash,u.status,COALESCE(array_agg(r.code) FILTER (WHERE r.code IS NOT NULL),'{}') roles
       FROM users u LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id
       WHERE u.email=$1 OR u.phone=$1 GROUP BY u.id`, [identifier.trim().toLowerCase()]
    );
    const user = result.rows[0];
    if (!user || user.status !== 'ACTIVE' || !user.password_hash || !(await bcrypt.compare(password, user.password_hash))) throw new Error('Invalid credentials');
  ...
  async issueSession(user: AuthenticatedUser, request: Request) {
    ...
    const accessToken = await this.jwt.signAsync({ sub: user.id, sessionId, roles: user.roles }, { secret: this.config.getOrThrow('JWT_ACCESS_SECRET'), expiresIn: this.config.get('JWT_ACCESS_TOKEN_TTL', '15m') });
    const refreshToken = randomBytes(48).toString('base64url');
    const refreshHash = this.hashToken(refreshToken);
    const expiresAt = new Date(Date.now() + this.durationMs(this.config.get('JWT_REFRESH_TOKEN_TTL', '7d')));
    await this.db.query(`INSERT INTO auth_sessions (id,user_id,refresh_token_hash,user_agent,ip_address,expires_at) VALUES ($1,$2,$3,$4,$5,$6)`, [sessionId, user.id, refreshHash, request.headers['user-agent'] ?? null, request.ip || null, expiresAt]);
  ...
  async refresh(refreshToken: string, request: Request) {
    ...
    const result = await this.db.query<AuthSession & { email: string | null; phone: string | null; roles: UserRole[] }>(
      `SELECT s.id,s.user_id,s.refresh_token_hash,s.expires_at,s.revoked_at,u.email,u.phone,COALESCE(array_agg(r.code) FILTER (WHERE r.code IS NOT NULL),'{}') roles
       FROM auth_sessions s JOIN users u ON u.id=s.user_id LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN roles r ON r.id=ur.role_id
       WHERE s.refresh_token_hash=$1 GROUP BY s.id,u.id`, [hash]);
    const session = result.rows[0];
    if (!session || session.revoked_at || new Date(session.expires_at) <= new Date()) throw new Error('Invalid refresh token');
