import { ApiError, api, type AuthUser, type AuthSessionResponse } from '@/lib/api';

export type AuthStorage = { accessToken: string; refreshToken: string; user: AuthUser };
...
export function readAuthStorage(): AuthStorage | null {
  ...
  try { return { accessToken, refreshToken, user: JSON.parse(rawUser) as AuthUser }; } catch { clearAuthStorage(); return null; }
}

export function writeAuthStorage(session: AuthSessionResponse) {
...
export async function authenticatedFetch<T>(path: string, init: RequestInit = {}): Promise<T> {
  try { return await fetchJson<T>(path, init); }
  catch (error) {
...
export class ApiError extends Error { constructor(public readonly status: number, message: string, public readonly errors?: string[]) { super(message); this.name = 'ApiError'; } }
export interface AuthUser { id: string; email?: string | null; phone?: string | null; roles: string[]; }
export interface AuthSessionResponse { accessToken: string; refreshToken: string; user: AuthUser; }
export async function fetchJson<T>(path: string, init: RequestInit = {}): Promise<T> {
  const accessToken = typeof window !== 'undefined' ? window.sessionStorage.getItem('vtu_access_token') : null;
  const response = await fetch(`${API_BASE_URL}${path}`, { ...init, credentials: 'include', headers: { Accept: 'application/json', 'Content-Type': 'application/json', ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}), ...(init.headers ?? {}) } });
...
export const api = {
  ...
  me: () => authenticatedFetch<AuthUser>('/auth/me'),
